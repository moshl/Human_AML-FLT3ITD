...
my $tempfile=$ARGV[0];
open IN,"$tempfile";
my @type=('PET_MF','PET_PV','PPV_MF');
while (<IN>) {
	chomp $_;	
	my $disease=(split /\t/,$_)[2];
	#my $num_D = grep /$disease/g, @type; 
	if (grep {$_ eq $disease} @type){
	#print $disease,"\n";
	$idx=(split /\t/,$_)[1];
	$hash{$idx}=1;
	}
...
##run dictionary: eg. /p200/liuxin_group/moshl/Temp/TCGA/HM/MPN
##usage perl ../../Code/SplitMAF_HM_MPN_bySample.pl ./data_JAK2_patients_fromPaper_TableS2.txt ./data_clinical_sample.txt ./data_mutations_mskcc.txt
